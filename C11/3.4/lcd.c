...
lcd display = {LCDWIDTH, LCDHEIGHT, East, 0, 0, WHITE, BLACK};
...
void display_char(char c)
{
	...
	//this loop sets where the corresponding colours to create a character go
	for(x=sc; x<=ec; x++) {
		write_cmd(COLUMN_ADDRESS_SET);
		write_data16(x);
		write_data16(x);
	...
	//writes the data into the memory
	write_cmd(COLUMN_ADDRESS_SET);
	write_data16(x);
	write_data16(x);
	write_cmd(MEMORY_WRITE);
	...
	//prepares the display for a future character by adding 6
	display.x += 6;
	
	//checks if the character will be written out of screen and if yes will restart it and move to next row
